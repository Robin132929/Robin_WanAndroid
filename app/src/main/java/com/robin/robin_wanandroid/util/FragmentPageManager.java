...
public class FragmentPageManager {
    /**
     * true 为不需要加入到 Fragment容器进行统一管理,默认为 false
     */
    public static final String IS_NOT_ADD_ACTIVITY_LIST = "is_not_add_fragment_list";
    private static volatile FragmentPageManager sFragmentPageManager;
    protected final String TAG = this.getClass().getSimpleName();
    private Application mApplication;
    /**
     * 管理所有存活的  Fragment, 容器中的顺序仅仅是  Fragment 的创建顺序, 并不能保证和  Fragment 任务栈顺序一致
     */
    ...
    public static FragmentPageManager getFragmentPageManager() {
    ...
    public FragmentPageManager init(Application application) {
    ...
    public void showSnackbar(String message, boolean isLong) {
        if (getCurrentFragment() == null && getTopFragment() == null) {
            Timber.tag(TAG).w("mCurrentFragment == null when showSnackbar(String,boolean)");
            return;
        }
    ...
    public Fragment getTopFragment() {
        ...
        return mFragmentList.size() > 0 ? mFragmentList.get(mFragmentList.size() - 1) : null;
    }
...
//    }
    ...
    public void removeFragment(Fragment fragment) {
        if (mFragmentList == null) {
            Timber.tag(TAG).w("mFragmentList == null when removeFragment(Fragment)");
            return;
        }
        synchronized (FragmentPageManager.class) {
     ...
     * @param location
     */
    public Fragment removeFragment(int location) {
        if (mFragmentList == null) {
            Timber.tag(TAG).w("mFragmentList == null when removeFragment(int)");
            return null;
        }
        synchronized (FragmentPageManager.class) {
            if (location > 0 && location < mFragmentList.size()) {
                return mFragmentList.remove(location);
            }
        }
        return null;
    }
...
//    }
    ...
    public void killAll(Class<?>... excludeFragmentClasses) {
        List<Class<?>> excludeList = Arrays.asList(excludeFragmentClasses);
        synchronized (FragmentPageManager.class) {
            Iterator<Fragment> iterator = getFragmentList().iterator();
            while (iterator.hasNext()) {
     ...
     *
     * @param excludeFragmentName {@link Activity} 的完整全路径
     */
    public void killAll(String... excludeFragmentName) {
        List<String> excludeList = Arrays.asList(excludeFragmentName);
        synchronized (FragmentPageManager.class) {
            Iterator<Fragment> iterator = getFragmentList().iterator();
            while (iterator.hasNext()) {
                Fragment next = iterator.next();
