...
public final class ToastUtils {
    ...
    private static int yOffset         = (int) (64 * ContextUtil.getAppContext().getResources().getDisplayMetrics().density + 0.5);
    ...
    public static void setView(@LayoutRes int layoutId) {
        LayoutInflater inflate = (LayoutInflater) ContextUtil.getAppContext().getSystemService(Context.LAYOUT_INFLATER_SERVICE);
    ...
    private static void show(@StringRes int resId, int duration) {
        show(ContextUtil.getAppContext().getResources().getText(resId).toString(), duration);
    }
    ...
    private static void show(@StringRes int resId, int duration, Object... args) {
        show(String.format(ContextUtil.getAppContext().getResources().getString(resId), args), duration);
    }
    ...
    private static void show(CharSequence text, int duration) {
        ...
        if (sViewWeakReference != null) {
            final View view = sViewWeakReference.get();
            if (view != null) {
                sToast = new Toast(ContextUtil.getAppContext());
                sToast.setView(view);
        ...
        if (!isCustom) {
            if (messageColor != DEFAULT_COLOR) {
                ...
                sToast = Toast.makeText(ContextUtil.getAppContext(), spannableString, duration);
            } else {
                sToast = Toast.makeText(ContextUtil.getAppContext(), text, duration);
            }
        }
